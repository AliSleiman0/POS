...
| | |
|---|---|
| **Current phase** | **Phase 10 in progress: the whole server side works and nothing has a screen.** A shop can be seated, ordered for, fired to the kitchen, split and settled — every one of those through the API, none of them through a browser. 1758 .NET + 321 Vitest green locally; **nothing has run in CI**, the branch is unpushed. |
| **Next up** | **10.7, the restaurant UI, with 10.9's e2e spec in the same piece of work** — per the rule below that UI testing never accumulates. `Pos.Seed --restaurant` now seeds a room, a kitchen and a menu, so there is finally something to build against and click through. After that: **a real shop.** `DECISIONS.md` has argued for that since Phase 8 and nobody who has worked a till or a pass has touched any of this. |
...
- [ ] **10.7 Restaurant UI** — **not built, and now the whole of what is left.** `/register` still renders the retail screen for every tenant. Every endpoint works and none has a caller, so **a shop cannot use any of this today**.
...
- [ ] **10.9 Tests and the seed fixture** — 317 new .NET tests landed with the milestones above. **`Pos.Seed --restaurant` now exists**: three stations, two areas with eight tables, a nine-item menu routed through its categories and two modifier groups, one required — so the room can be worked by hand. Missing: `restaurant.spec.ts`, which belongs with 10.7 rather than after it.

**Phase 10 is in progress and its gaps are listed in the phase doc rather than hidden.** The server side of a service — seat, order, fire, bump, split, pay — is complete and tested against a real Postgres, and a seeded room exists to exercise it in. **Every screen is still missing**, and that is now the only substantial thing between this phase and a restaurant that could use it.
