...
| 10 | [Restaurant mode](phases/PHASE-10-restaurant.md) | Tables, orders, modifiers, kitchen display, split bills, tips | ✅ Done |
...
- [x] **10.4 Courses, seats, stations, kitchen display** — `Station`, `KitchenTicket`/`KitchenTicketLine`, `POST /orders/{id}/fire`, and the display's list, bump and recall. **Routing is set on a category and inherited** — `StationRouting` walks up from the product and takes the first station it finds, because a restaurant will configure eight categories and will not configure four hundred products; `Product.StationId` is the override. **Unrouted refuses the whole fire, naming the product**, rather than falling back to a default station that would send a steak to the bar silently. **Firing is idempotent by the line's status, not by the key** — changed in the same transaction that writes the ticket, so a second handheld carrying its own key creates nothing; proved concurrently. A ticket is **append-only**: a line voided afterwards is struck through on the screen, never edited out, because the chef may already have plated it. Nothing here is audited — firing moves no money. The display **polls**, at a stated and tunable five seconds; no transport was added.
...
- [x] **10.7 Restaurant UI** — `features/restaurant/`: floor, order screen with courses and a modifier sheet, bill screen reusing the retail cash pad, and the pass as a **kiosk outside `AppLayout`** because a kitchen display is an appliance rather than a page. `/register` picks the screen from `serviceMode` — one route, because staff are trained on "the register" — and `RegisterPage` is wrapped, never edited. Order state is server-owned through TanStack Query, a deliberate departure from `CartProvider`/`sessionStorage` because a table has to be visible from a second device; nothing new reaches storage. **Two gaps fell out of building it**: `POST .../pay` returned no change due, and the restaurant never offered to open a drawer — a shop could seat, order and fire all evening and find out at the first bill. Both fixed.
- [x] **10.8 Reporting** — takings by table and by server, tips by server, spend per cover and table turn, joined `OrderBill → Sale` and **never to current catalog data**. The queries run only for a restaurant, so a counter's report costs what it always did, and the sections come back **empty rather than absent** so no client branches on a nullable. Spend per cover is **null where nobody keyed a cover count** — a takeaway has none, and dividing by a missing denominator would put a plausible, wrong number on a report somebody decides by.
- [x] **10.9 Tests and the seed fixture** — `Pos.Seed --restaurant` seeds a room, a kitchen and a menu; `restaurant.spec.ts` walks the phase doc's nine-step verification in a browser against **a second e2e tenant**, so no retail spec is disturbed and the last assertion can be that the retail shop is untouched. **One test rather than one per screen**: a test per screen depends on the last one's leftovers and the database outlives the run. The spec is what found the missing drawer control.

**Phase 10 is complete, and what remains is listed in the phase doc rather than hidden.** The largest item is not code: nobody who has worked a restaurant has used any of it, and the floor, the modifier sheet and the pass are guesses about how a room works until somebody who works one disagrees. The tender pad's provisional change ignoring the tip is the one defect worth fixing before a real service.
